...
TEST(reduce_connection_dead_forces_none) {
    ...
    TypioWlDesiredState d = typio_wl_focus_reduce(&facts, &prev);
...
TEST(reduce_done_committed_deactivate_to_soft_pause) {
    /* Regression: the input-method-v2 protocol is double-buffered, so a
     * deactivate is committed by a done() that clears the raw
     * im_deactivate_seen in the same dispatch batch. reduce must still
     * soft-pause via the batch-surviving im_done_had_deactivate flag —
     * otherwise a plain focus-out (click away to a non-editable) is lost and
     * the grab stays YES, leaving the engine composing while unfocused. */
    TypioWlInputFacts facts = {.connection_alive = true, .im_done_had_deactivate = true};
    TypioWlDesiredState prev = {.grab = TYPIO_WL_GRAB_WANT_YES};
    TypioWlDesiredState d = typio_wl_focus_reduce(&facts, &prev);
    ASSERT(d.grab == TYPIO_WL_GRAB_WANT_SOFT_PAUSE);
...
TEST(reduce_batch_with_activate_and_deactivate_stays_yes) {
    /* Focus move: one done() batch carries both a deactivate (old field) and
     * an activate (new field). Activate wins — the grab stays YES and the
     * move reactivates rather than soft-pausing. */
...
TEST(diff_none_with_ready_destroys) {
    ...
    ASSERT(e.discard_composition == true);
...
TEST(diff_focus_out_discards_composition) {
    /* Soft-pause defocus (deactivate): the grab is retained (no destroy),
     * but the in-flight composition must still be discarded and the
     * compositor preedit blanked so it cannot leak into the next field. */
    TypioWlDesiredState d = {.grab = TYPIO_WL_GRAB_WANT_SOFT_PAUSE, .focus_out = true};
    TypioWlActualState  a = {.grab = TYPIO_WL_GRAB_RES_READY};
    TypioWlEffectSet e = typio_wl_focus_diff(&d, &a);
    ASSERT(e.destroy_grab == false);
...
TEST(diff_no_focus_change_keeps_composition) {
    /* Stable YES + READY with no focus edge: never tear down the live
     * composition. discard_composition must stay false. */
    TypioWlDesiredState d = {.grab = TYPIO_WL_GRAB_WANT_YES};
    TypioWlActualState  a = {.grab = TYPIO_WL_GRAB_RES_READY};
    TypioWlEffectSet e = typio_wl_focus_diff(&d, &a);
    ASSERT(e.discard_composition == false);
    ASSERT(e.clear_preedit == false);
}
...
TEST(diff_idempotent_stable_none) {
    ...
    TypioWlDesiredState d = {.grab = TYPIO_WL_GRAB_WANT_NONE};
    TypioWlActualState  a = {.grab = TYPIO_WL_GRAB_RES_ABSENT};
    TypioWlEffectSet e1 = typio_wl_focus_diff(&d, &a);
    TypioWlEffectSet e2 = typio_wl_focus_diff(&d, &a);
...
TEST(reduce_null_returns_safe_defaults) {
    TypioWlDesiredState d = typio_wl_focus_reduce(NULL, NULL);
    ASSERT(d.grab == TYPIO_WL_GRAB_WANT_NONE);
    ASSERT(d.focus_in == false);
    ASSERT(d.focus_out == false);
}
...
TEST(diff_null_returns_empty_effects) {
    TypioWlEffectSet e = typio_wl_focus_diff(NULL, NULL);
    ASSERT(e.destroy_grab == false);
    ASSERT(e.create_grab == false);
}
...
int main(void)
{
    printf("Running focus controller tests:\n");
    ...
    run_test_reduce_done_committed_deactivate_to_soft_pause();
    run_test_reduce_batch_with_activate_and_deactivate_stays_yes();
